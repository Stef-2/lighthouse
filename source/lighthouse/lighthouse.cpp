...
namespace lh
{
	...
	auto memory_suballocator<A>::request_and_commit_suballocation(const std::size_t size) -> void*
	{
		...
		for (auto it = m_free_memory_blocks.begin(); it != m_free_memory_blocks.end(); ++it)
			if (size <= it->m_size)
			{
				...
				iterator = it;
				iterator->m_offset += size;
				iterator->m_size -= size;
		...
		if (iterator != m_free_memory_blocks.end()) [[likely]]
			...
			if (iterator->m_size == 0)
				...
				std::ranges::sort(m_free_memory_blocks.begin() /*iterator*/,
								  m_free_memory_blocks.end(),
	...
	auto memory_suballocator<A>::free_suballocation(const memory_block& memory_block) -> void
	{
		...
		if (memory_block.m_size == 0) abort();
		...
		for (i = 0; i < m_free_memory_blocks.size(); i++)
		{
		...
		const auto it = i == m_free_memory_blocks.size() - 1 ? m_free_memory_blocks.end()
															 : m_free_memory_blocks.begin() + i;
...
auto main() -> int
{
	lh::memory_pool pool(1'000'000'000);
	...
	auto us = new std::unordered_set<int> {};
