...
namespace lh
{
	namespace vulkan
	{
		...
		debug_messanger::debug_messanger(const vk::raii::Instance& instance, const create_info& create_info)
			: raii_wrapper(instance.createDebugUtilsMessengerEXT(create_info.m_debug_info))
		{}

		VKAPI_ATTR auto VKAPI_CALL
		debug_messanger::debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
										VkDebugUtilsMessageTypeFlagsEXT message_type,
										const VkDebugUtilsMessengerCallbackDataEXT* callback_data,
										void* user_data) -> VkBool32
		{
			if constexpr (debug_messanger::m_only_report_once)
				if (std::ranges::contains(debug_messanger::m_previously_reported_messages,
