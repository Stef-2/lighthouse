...
namespace lh
{
	namespace vulkan
	{
			...
			: m_image {nullptr}
		{
			...
			const auto memory_to_image_copy = vk::MemoryToImageCopyEXT {image_data,
													   static_cast<std::uint32_t>(width),
													   static_cast<std::uint32_t>(height),
													   vk::ImageSubresourceLayers {create_info.m_image_create_info.m_image_aspect, create_info.m_image_create_info.m_mip_levels - 1, 0, 1},
													   vk::Offset3D {0, 0, 0},
													   vk::Extent3D {static_cast<std::uint32_t>(width),
																	 static_cast<std::uint32_t>(height),
																	 1}};

			logical_device->copyMemoryToImageEXT({vk::CopyMemoryToImageInfoEXT {
				{vk::HostImageCopyFlagBitsEXT::eMemcpy}, **m_image, vk::ImageLayout::eReadOnlyOptimal, memory_to_image_copy}});
		}
	}
}
