...
lh::version::operator lh::output::string_t() const
{
	return std::to_string(major()) + ':' + std::to_string(minor()) + ':' + std::to_string(patch());
}

lh::version::version(version_t major, version_t minor, version_t patch) : m_version(VK_MAKE_VERSION(major, minor, patch)) {}
...
lh::version::version(packed_version_t version)
	: m_version(version)
{}
